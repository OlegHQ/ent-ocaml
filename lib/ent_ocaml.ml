...
module Edge_chain = struct
  let start edge_query = { chain_first = edge_query; chain_rest = [] }

  let then_ ?as_ ?target_query ~edge ~target chain =
    let target_query =
      Option.value target_query ~default:(Query.make target)
    in
    let step =
      {
        chain_edge = edge;
        chain_edge_alias = as_;
        chain_target = target;
        chain_target_query = target_query;
      }
    in
    { chain with chain_rest = chain.chain_rest @ [ step ] }

  let source chain = chain.chain_first.source

  let target chain =
    match List.rev chain.chain_rest with
    | [] -> chain.chain_first.target
    | step :: _ -> step.chain_target
end
...
module Edge_load = struct
  ...
  let group edge_query rows =
    {
      loaded_group_edge = edge_query.edge;
      loaded_group_alias = edge_query.edge_alias;
      loaded_group_name = name edge_query;
      loaded_group_rows = rows;
    }
  ...
  let case edge_query ~decode_target ~map =
    Loaded_edge_case
      {
        edge_case_query = edge_query;
        edge_case_decode_target = decode_target;
        edge_case_map = map;
      }
end
...
module Entql = struct
  ...
  let edge_id_type (entity : entity) edge_name =
    ...
    | None ->
        bad
          ("edge path currently requires a stored foreign-key edge: "
         ^ edge_name ^ ".id")
    | Some storage_key -> (
        match
          List.find_opt
            (fun (field : field) -> field.name = storage_key || field.storage_key = storage_key)
            entity.fields
        with
        | Some field -> Ok field.typ
        | None ->
            bad
  ...
  let edge_id_predicate edge_name op value =
    let field = "id" in
    match op with
    | Dynamic_filter.Equal -> Ok (Has_edge_with (edge_name, [ Eq (field, value) ]))
    | Not_equal -> Ok (Has_edge_with (edge_name, [ Neq (field, value) ]))
    | Greater_than -> Ok (Has_edge_with (edge_name, [ Gt (field, value) ]))
    | Greater_or_equal -> Ok (Has_edge_with (edge_name, [ Gte (field, value) ]))
    | Less_than -> Ok (Has_edge_with (edge_name, [ Lt (field, value) ]))
    | Less_or_equal -> Ok (Has_edge_with (edge_name, [ Lte (field, value) ]))
    | In_list -> (
        match value with
        | V_list values -> Ok (Has_edge_with (edge_name, [ In (field, values) ]))
        | _ -> bad ("expected list value for edge path: " ^ edge_name ^ ".id"))
    | Not_in_list -> (
        match value with
        | V_list values ->
            Ok (Has_edge_with (edge_name, [ Not_in (field, values) ]))
        | _ -> bad ("expected list value for edge path: " ^ edge_name ^ ".id"))
    | Contains | Has_prefix | Has_suffix | Is_null | Not_null ->
