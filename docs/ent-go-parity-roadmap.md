...
- `[@@deriving ent]` generation for entity metadata, functional query helpers,
  ...
  `Store.With_hooks` and `Client.With_hooks` modules wrap mutation execution
  ...
  metadata-validated single-entity filter strings with `&&`, `||`, parentheses,
  negation, equality, comparison, membership, string, and null operators into
...
| Eager loading | Stored-FK to-one `with_<edge>` loading and alias metadata implemented; nested and multi-edge loading pending | Batch secondary queries; named loaders |
| Named edges | Generated `with_<edge> ~as_` alias metadata implemented; named result maps pending | Map from edge name/alias to loaded rows |
...
| Hooks | Generated mutation middleware via `Store.With_hooks`, `Client.With_hooks`, and schema `Store.Schema_hooks` implemented; deterministic cross-source/global registration pending | Around generated mutators |
...
| Dynamic EntQL | Metadata-validated runtime field filters and a result-returning boolean expression parser implemented; richer cross-entity/path grammar pending | Runtime predicate AST parser/builder |
...
7. Eager loading and named edges:
   ...
   Store executors, and generated `with_<edge> ~as_` preserves named-edge alias
   metadata. Nested eager loading, multiple edge loads, per-edge limits,
   ordering, named result maps, and optional bidirectional backrefs remain.

8. Hooks, privacy, and interceptors:
   generated `Store.With_hooks` and `Client.With_hooks` modules wrap mutation
   execution with typed middleware, and generated `Store.With_policy` modules
   evaluate query and mutation privacy rule chains before backend execution.
   Generated `Store.With_interceptors` modules wrap read-path query execution. Schema
   attributes generate `Store.Schema_policy`, `Store.Schema_hooks`, and
   `Store.Schema_interceptors`. Traversal-specific interceptors,
   mixin-provided rules, and deterministic cross-source/global registration
   order remain.
...
11. EntQL and backend-specific metadata:
   ...
   predicates, an EntQL parser handles single-entity boolean expressions with
   `&&`, `||`, parentheses, negation, equality, comparison, membership, string,
   and null operators, generated JSON path predicates are implemented for JSON
   fields, and generated per-entity schema snapshots plus repository-wide
   snapshot manifests are implemented. Richer cross-entity/path grammar, custom
