...
- `[@@deriving ent]` generation for entity metadata, functional query helpers,
  ...
  edge alias/name metadata for each loaded row.
...
| Eager loading | Stored-FK to-one `with_<edge>` loading, alias metadata, and named loaded-edge records implemented; nested and multi-edge loading pending | Batch secondary queries; named loaders |
| Named edges | Generated `with_<edge> ~as_` alias metadata and single-edge named load results implemented; multi-edge result maps pending | Map from edge name/alias to loaded rows |
...
| Interceptors | Generated query middleware via `Store.With_interceptors`, `Client.With_interceptors`, `Store.Schema_interceptors`, and `Client.Schema_interceptors` implemented; traversal-specific interceptors pending | Around query execution and traversal construction |
...
7. Eager loading and named edges:
   ...
   metadata. Generated `load_edge_named` returns loaded-edge records carrying
   the edge alias/name for each row. Nested eager loading, multiple edge loads,
   per-edge limits, ordering, multi-edge result maps, and optional
...
8. Hooks, privacy, and interceptors:
   ...
   attributes generate Store and Client `Schema_policy`, `Schema_hooks`, and
   `Schema_interceptors` modules. Traversal-specific interceptors,
